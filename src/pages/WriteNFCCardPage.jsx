...
import {
  ...
  Smartphone, Leaf, ArrowUpRight, ChevronDown, ChevronUp,
...
const RECENT_ACTIVITY = [
  { Icon: Link2,    label: "URL Card Written",      time: "2 min ago",   type: "url"   },
  { Icon: User,     label: "vCard Card Written",    time: "15 min ago",  type: "vcard" },
  { Icon: Wifi,     label: "WiFi Card Written",     time: "32 min ago",  type: "wifi"  },
  { Icon: FileText, label: "Text Card Written",     time: "1 hour ago",  type: "text"  },
  { Icon: Mail,     label: "Email Card Written",    time: "2 hours ago", type: "email" },
  { Icon: MapPin,   label: "Location Card Written", time: "3 hours ago", type: "loc"   },
];
      ...
      {steps.map((step, i) => {
        ...
        return (
          <div key={step} className="flex items-center gap-0">
            <div className={`flex items-center gap-1.5 px-2.5 sm:px-4 py-2 rounded-full text-xs font-semibold transition-all whitespace-nowrap ${curr ? "bg-red-600 text-white shadow-lg shadow-red-200" : done ? "bg-green-100 text-green-700 border border-green-200" : "text-red-300"}`}>
              <span className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold flex-shrink-0 border ${curr ? "bg-white text-red-600 border-white" : done ? "bg-green-500 border-green-400 text-white" : "border-red-200 text-red-400"}`}>
                {done ? <CheckCircle2 className="w-3 h-3" /> : i + 1}
              </span>
              <span className="hidden sm:inline">{step}</span>
            </div>
            {i < steps.length - 1 && <div className={`w-4 sm:w-8 h-px mx-0.5 sm:mx-1 flex-shrink-0 ${done ? "bg-green-400" : "bg-red-100"}`} />}
          </div>
        );
      })}
    </div>
...
function WriteNFCForm({ selType, setSelType, fieldVal, setFieldVal, step, setStep, nfcStatus, cardInfo, advOpts, setAdvOpts, handleWrite, handleClear }) {
  ...
  const statusBg = { [NFC.SUCCESS]: "bg-green-50 border-green-300 text-green-700", [NFC.ERROR]: "bg-red-100 border-red-400 text-red-700", [NFC.UNSUPPORTED]: "bg-rose-50 border-rose-300 text-rose-600" }[nfcStatus] || "bg-red-50 border-red-200 text-red-600";
  ...
  return (
    <div className="bg-white border border-red-100 rounded-xl p-4 sm:p-5 shadow-sm">
      ...
      <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-2 mb-5">
        {DATA_TYPES.map((d, i) => (
          <button key={d.id} onClick={() => handleSelectType(i)}
            className={`flex items-center gap-1.5 sm:gap-2 px-2 sm:px-3 py-2.5 rounded-xl border text-left transition-all ${selType === i ? "bg-red-600 border-red-600 text-white shadow-md shadow-red-200" : "bg-red-50 border-red-200 text-red-700 hover:border-red-400 hover:bg-red-100"}`}>
        ...
        <div className="space-y-3">
          {fields.map(f => (
            <div key={f.key}>
              ...
              {f.multiline
                ? <textarea value={fieldVal[f.key] || ""} onChange={e => handleFieldChange({ ...fieldVal, [f.key]: e.target.value })} placeholder={f.placeholder} rows={3} className={`${inputCls} resize-none`} />
                : <input type={f.type || "text"} value={fieldVal[f.key] || ""} onChange={e => handleFieldChange({ ...fieldVal, [f.key]: e.target.value })} placeholder={f.placeholder} className={inputCls} />
              }
            </div>
          ))}
        </div>
      ...
      <div className="mt-4 pt-4 border-t border-red-100">
        <h3 className="text-red-900 text-sm font-semibold mb-3 flex items-center gap-2"><Settings className="w-4 h-4 text-red-500" />Advanced Options</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-2.5 gap-x-6">
          {[{ key: "dynamicLink", label: "Enable Dynamic Link" }, { key: "lockAfterWrite", label: "Lock Card After Write" }, { key: "passwordProtect", label: "Set Password Protection" }, { key: "autoDetect", label: "Auto Detect Card Type" }, { key: "writeOnce", label: "Write Once (Lock after)" }, { key: "optimizeMemory", label: "Optimize Memory Usage" }].map(opt => (
            <label key={opt.key} className="flex items-center gap-2 cursor-pointer group">
              <input type="checkbox" checked={advOpts[opt.key] || false} onChange={e => setAdvOpts({ ...advOpts, [opt.key]: e.target.checked })} className="w-4 h-4 rounded accent-red-500 cursor-pointer" />
      ...
      <div className="mt-4 pt-4 border-t border-red-100">
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-4">
          <div className="flex items-center gap-3 bg-red-50 border border-red-200 rounded-xl px-4 py-3 flex-1">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-red-500 to-rose-600 flex items-center justify-center shadow-md shadow-red-100 flex-shrink-0"><dt.Icon className="w-5 h-5 text-white" /></div>
            ...
            <span className={`text-xs font-bold px-2 py-0.5 rounded-full border flex-shrink-0 ${isReady ? "bg-green-100 text-green-700 border-green-200" : "bg-red-100 text-red-400 border-red-200"}`}>{isReady ? "Ready" : "Incomplete"}</span>
          </div>
          <div className="bg-red-50 border border-red-200 rounded-xl px-4 py-3 text-xs space-y-1.5 flex-shrink-0">
            {[{ l: "Data Size", v: `${Math.max(28, (primVal?.length || 0) + 10)} B` }, { l: "Writable", v: "Yes" }, { l: "Card Type", v: "Auto" }].map(r => (
              <div key={r.l} className="flex items-center gap-4"><span className="text-red-400 w-20">{r.l}</span><span className="text-red-900 font-medium">{r.v}</span></div>
            ))}
          </div>
        </div>
        <div className="flex gap-3">
          <button onClick={handleWrite} disabled={!isReady || isBusy}
            ...
            {isBusy ? <><RefreshCw className="w-4 h-4 animate-spin" />{nfcStatus === NFC.SCANNING ? "Scanning..." : "Writing..."}</> : nfcStatus === NFC.SUCCESS ? <><CheckCircle2 className="w-4 h-4" />Written Successfully!</> : <><Radio className="w-4 h-4" />Write to NFC Card</>}
          </button>
          <button onClick={handleClear} className="flex items-center gap-2 bg-white hover:bg-red-50 border border-red-200 hover:border-red-400 text-red-600 text-sm font-semibold px-4 sm:px-5 py-3 rounded-xl transition-all">
        ...
        {nfcStatus !== NFC.IDLE && (
          <div className={`mt-3 flex items-center gap-2 text-xs font-medium px-4 py-2.5 rounded-xl border ${statusBg}`}>
            {nfcStatus === NFC.SUCCESS ? <CheckCircle2 className="w-4 h-4 flex-shrink-0" /> : nfcStatus === NFC.ERROR || nfcStatus === NFC.UNSUPPORTED ? <AlertTriangle className="w-4 h-4 flex-shrink-0" /> : <RefreshCw className="w-4 h-4 flex-shrink-0 animate-spin" />}
    ...
    try {
      ...
      ndef.addEventListener("reading", ({ message, serialNumber }) => {
        const records = message.records.map(r => {
          ...
          return { type: r.recordType, data: decoder.decode(r.data) };
        });
    ...
    <div className="bg-white border border-red-100 rounded-xl p-4 sm:p-5 shadow-sm">
      ...
      <div className="flex justify-center mb-6">
        <div className="relative w-36 h-36">
          ...
          <div className={`absolute inset-4 rounded-full border-2 flex items-center justify-center transition-all ${isSuccess ? "border-green-400 bg-green-50" : isScanning ? "border-red-500 bg-red-50 animate-pulse" : "border-red-200 bg-red-50/50"}`}>
            <ScanLine className={`w-12 h-12 transition-colors ${isSuccess ? "text-green-500" : isScanning ? "text-red-500" : "text-red-300"}`} />
          </div>
          {isSuccess && <div className="absolute top-1 right-1 w-6 h-6 bg-green-500 rounded-full flex items-center justify-center"><CheckCircle2 className="w-4 h-4 text-white" /></div>}
        </div>
      </div>

      {/* Status badge */}
      <div className="flex justify-center mb-5">
        <div className={`flex items-center gap-1.5 border text-xs font-semibold px-4 py-1.5 rounded-full ${isSuccess ? "bg-green-100 border-green-300 text-green-700" : isScanning ? "bg-red-100 border-red-300 text-red-600" : scanStatus === "error" || scanStatus === "unsupported" ? "bg-rose-100 border-rose-300 text-rose-600" : "bg-red-50 border-red-200 text-red-500"}`}>
      ...
      {scannedData && (
        <div className={`mb-4 p-4 rounded-xl border ${isSuccess ? "bg-green-50 border-green-200" : "bg-red-50 border-red-200"}`}>
          {scannedData.error ? (
            <div className="text-red-700 text-xs flex items-center gap-2"><AlertTriangle className="w-4 h-4 flex-shrink-0" />{scannedData.error}</div>
          ) : (
            <div className="space-y-2">
              <div className="flex justify-between text-xs"><span className="text-green-600 font-semibold">UID</span><span className="text-green-900 font-mono">{scannedData.uid || "N/A"}</span></div>
              <div className="flex justify-between text-xs"><span className="text-green-600 font-semibold">Read at</span><span className="text-green-900">{scannedData.time}</span></div>
              <div className="flex justify-between text-xs"><span className="text-green-600 font-semibold">Records</span><span className="text-green-900">{scannedData.records?.length || 0}</span></div>
      ...
      <div className="mt-4 bg-red-50 border border-red-100 rounded-xl p-3">
        ...
        <div className="flex flex-wrap gap-1">
          {SUPPORTED_CARDS.map(c => <span key={c} className="text-[9px] font-medium text-red-600 bg-white border border-red-200 px-1.5 py-0.5 rounded-md">{c}</span>)}
        </div>
      </div>
    </div>
  );
...
function BulkWritePanel({ onBack }) {
  ...
  return (
    <div className="bg-white border border-red-100 rounded-xl p-4 sm:p-5 shadow-sm">
      ...
      <div className="mb-3">
        ...
        <textarea value={csvData} onChange={e => setCsvData(e.target.value)} rows={5} placeholder={`type,value\nurl,https://example.com\nvcard,John Doe`}
        ...
        <div className="mb-3">
          ...
          {writing && (
            <div className="mb-3">
              ...
              <div className="h-2 bg-red-100 rounded-full overflow-hidden"><div className="h-full bg-gradient-to-r from-red-500 to-rose-500 rounded-full transition-all duration-500" style={{ width: `${progress}%` }} /></div>
            </div>
          )}
          {done && <div className="flex items-center gap-2 bg-green-50 border border-green-200 rounded-lg px-3 py-2 mb-3 text-green-700 text-xs font-semibold"><CheckCircle2 className="w-4 h-4" />All {parsed.length} cards written successfully!</div>}
          <div className="space-y-1.5 max-h-48 overflow-y-auto pr-1">
            {parsed.map(row => (
              <div key={row.id} className="flex items-center gap-2 bg-red-50 border border-red-100 rounded-lg px-3 py-2">
                ...
                {row.status === "success" ? <CheckCircle2 className="w-3.5 h-3.5 text-green-500 flex-shrink-0" /> : <div className="w-3.5 h-3.5 rounded-full border-2 border-red-200 flex-shrink-0" />}
              </div>
            ))}
          </div>
        </div>
      ...
      <button onClick={handleBulkWrite} disabled={!parsed.length || writing}
        ...
        {writing ? <><RefreshCw className="w-4 h-4 animate-spin" />Writing {progress}%...</> : done ? <><CheckCircle2 className="w-4 h-4" />Write Again</> : <><Package className="w-4 h-4" />Start Bulk Write</>}
      </button>
    </div>
  );
}
...
function CreateTemplatePanel({ onBack }) {
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white border border-red-100 rounded-xl p-4 sm:p-5 shadow-sm">
        <div className="flex items-center gap-3 mb-5">
          ...
          <div><h2 className="text-red-900 font-bold text-lg">Create Template</h2><p className="text-red-400 text-xs">Save reusable NFC card templates</p></div>
        </div>
        <div className="space-y-3">
          <div>
            ...
            <div className="grid grid-cols-3 sm:grid-cols-4 gap-2">
              {DATA_TYPES.slice(0, 8).map((d, i) => (
                <button key={d.id} onClick={() => setSelType(i)}
                  className={`flex items-center gap-1.5 px-2 py-2 rounded-xl border text-left transition-all ${selType === i ? "bg-red-600 border-red-600 text-white shadow-md shadow-red-200" : "bg-red-50 border-red-200 text-red-700 hover:border-red-400"}`}>
                  <d.Icon className="w-3.5 h-3.5 flex-shrink-0" />
        ...
        {saved && <div className="mt-3 flex items-center gap-2 bg-green-50 border border-green-200 rounded-lg px-3 py-2 text-green-700 text-xs font-semibold"><CheckCircle2 className="w-4 h-4" />Template saved!</div>}
      ...
      <div className="bg-white border border-red-100 rounded-xl p-4 sm:p-5 shadow-sm">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          {templates.map(t => (
            <div key={t.id} className="flex items-center gap-3 bg-red-50 border border-red-100 rounded-xl px-3 py-2.5 hover:border-red-300 transition-colors cursor-pointer group">
              ...
              <button onClick={() => setTemplates(p => p.filter(x => x.id !== t.id))} className="text-red-300 hover:text-red-600 opacity-0 group-hover:opacity-100 transition-all flex-shrink-0">
...
function ReportsPanel({ onBack }) {
  const handleExport = () => {
    ...
    a.download = "nfc_write_analytics.csv"; a.click();
  };
      ...
      <div className="bg-white border border-red-100 rounded-xl p-4 sm:p-5 shadow-sm">
        <div className="flex items-center justify-between mb-5">
          <div className="flex items-center gap-3">
            ...
            <div><h2 className="text-red-900 font-bold text-lg">Write Reports</h2><p className="text-red-400 text-xs">Analytics for your NFC card writes</p></div>
          </div>
          <button onClick={handleExport} className="flex items-center gap-1.5 text-xs px-3 py-2 border border-red-200 rounded-xl text-red-600 hover:bg-red-50 transition-all">
        ...
        <div>
          ...
          <ResponsiveContainer width="100%" height={130}>
            <AreaChart data={SCAN_CHART} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
              ...
              <YAxis tick={{ fontSize: 8, fill: "#f87171" }} axisLine={false} tickLine={false} />
              <Tooltip {...TOOLTIP_STYLE} />
              <Area type="monotone" dataKey="v" name="Scans" stroke="#ef4444" strokeWidth={2} fill="url(#scanGradR)" dot={{ r: 3, fill: "#ef4444", stroke: "#fff", strokeWidth: 1.5 }} />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      </div>
        ...
        <div className="space-y-2.5">
          {RECENT_ACTIVITY.map((a, i) => (
            <div key={i} className="flex items-center gap-2.5 p-2.5 bg-red-50/50 border border-red-100 rounded-xl">
              ...
              <span className="text-[10px] bg-green-100 text-green-700 border border-green-200 px-2 py-0.5 rounded-full font-semibold flex items-center gap-0.5"><CheckCircle2 className="w-2.5 h-2.5" />Success</span>
    ...
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center p-3 sm:p-4 overflow-y-auto">
      <div className="bg-white rounded-2xl w-full max-w-3xl shadow-2xl border border-red-100 my-auto">
        ...
        <div className="bg-gradient-to-r from-red-600 to-rose-600 rounded-t-2xl px-5 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl bg-white/20 border border-white/30 flex items-center justify-center"><Crown className="w-5 h-5 text-white" /></div>
            <div>
          ...
          <button onClick={onClose} className="w-8 h-8 rounded-full bg-white/20 hover:bg-white/30 flex items-center justify-center text-white transition-all"><X className="w-4 h-4" /></button>
        </div>
        {/* Plans */}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-4">
            {PLANS.map(p => (
              <div key={p.name} onClick={() => setSelected(p.name)}
                ...
                {p.badge && (
                  <div className="absolute -top-2.5 left-1/2 -translate-x-1/2 bg-red-600 text-white text-[9px] font-bold px-2.5 py-0.5 rounded-full whitespace-nowrap shadow-md shadow-red-200">{p.badge}</div>
                )}
                ...
                {selected === p.name && <div className="absolute top-2 left-2 w-4 h-4 bg-red-600 rounded-full flex items-center justify-center"><Check className="w-2.5 h-2.5 text-white" /></div>}
              </div>
            ))}
          </div>
          <div className="flex gap-3">
...
function TapNFCSidePanel({ nfcStatus, cardInfo }) {
  ...
  return (
    <div className="bg-white border border-red-100 rounded-xl p-4 sm:p-5 shadow-sm">
      <h2 className="text-red-900 font-bold text-sm mb-0.5 flex items-center gap-2"><Radio className="w-4 h-4 text-red-500" />Tap Your NFC Card</h2>
      ...
      <div className="flex justify-center mb-4">
        <div className="relative w-28 h-28">
          ...
          <div className={`absolute inset-4 rounded-full border-2 flex items-center justify-center transition-all ${isSuccess ? "border-red-400 bg-red-50" : isActive ? "border-red-500 bg-red-50 animate-pulse" : "border-red-200 bg-red-50/50"}`}>
            <Smartphone className={`w-10 h-10 transition-colors ${isSuccess || isActive ? "text-red-500" : "text-red-300"}`} />
          </div>
        </div>
      </div>
      <div className="flex justify-center mb-4">
        <div className={`flex items-center gap-1.5 border text-xs font-semibold px-3 py-1.5 rounded-full ${isSuccess ? "bg-green-100 border-green-300 text-green-700" : isActive ? "bg-red-100 border-red-300 text-red-600" : "bg-red-50 border-red-200 text-red-500"}`}>
      ...
      <div className="flex flex-wrap gap-1 mb-4">
        {SUPPORTED_CARDS.map(c => <span key={c} className="text-[9px] font-medium text-red-600 bg-red-50 border border-red-200 px-1.5 py-0.5 rounded-md">{c}</span>)}
      </div>
      <div className="border-t border-red-100 pt-3">
        <p className="text-red-900 text-xs font-semibold mb-2 flex items-center gap-2"><Info className="w-3.5 h-3.5 text-red-500" />Card Information</p>
        <div className="space-y-1.5">
          {[{ l: "Status", v: cardInfo.status || "No Card Detected" }, { l: "UID", v: cardInfo.uid || "--" }, { l: "Card Type", v: cardInfo.cardType || "--" }, { l: "Memory", v: cardInfo.memorySize || "--" }, { l: "Writable", v: cardInfo.writable || "--" }, { l: "Lock Status", v: cardInfo.lockStatus || "--" }].map(r => (
      ...
      <div className="bg-white border border-red-100 rounded-xl p-4 shadow-sm">
        ...
        <ResponsiveContainer width="100%" height={80}>
          <AreaChart data={SCAN_CHART} margin={{ top: 2, right: 2, left: -32, bottom: 0 }}>
            ...
            <Tooltip {...TOOLTIP_STYLE} />
            <Area type="monotone" dataKey="v" name="Scans" stroke="#ef4444" strokeWidth={2} fill="url(#scanGradW)" dot={{ r: 3, fill: "#ef4444", stroke: "#fff", strokeWidth: 1.5 }} />
          </AreaChart>
        </ResponsiveContainer>
      </div>
        ...
        <div className="space-y-2.5">
          {RECENT_ACTIVITY.map((a, i) => (
            <div key={i} className="flex items-center gap-2.5">
              <div className="w-7 h-7 rounded-lg bg-red-100 flex items-center justify-center flex-shrink-0"><a.Icon className="w-3.5 h-3.5 text-red-600" /></div>
...
export default function WriteNFCCardPage({ onMenuClick }) {
  ...
  const handleWrite = useCallback(async () => {
    ...
    if (typeof NDEFReader === "undefined") { setNfcStatus(NFC.UNSUPPORTED); return; }
    try {
      ...
      setCardInfo({ status: "Written Successfully", uid: "Auto Detected", cardType: "NDEF Compatible", memorySize: `${Math.max(28, primVal.length + 10)} Bytes`, writable: "Yes", lockStatus: advOpts.lockAfterWrite ? "Locked" : "Unlocked" });
  ...
  const PANEL_TITLES = { write: "Write NFC Card", scan: "Scan NFC Card", bulk: "Bulk Write Cards", template: "Create Template", reports: "View Reports" };
        ...
        <div className="px-3 sm:px-5 pb-4">
          <div className="flex flex-col xl:flex-row gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              {activePanel === "scan"     && <ScanNFCPanel     onBack={() => setActivePanel("write")} />}
              {activePanel === "bulk"     && <BulkWritePanel   onBack={() => setActivePanel("write")} />}
              {activePanel === "template" && <CreateTemplatePanel onBack={() => setActivePanel("write")} />}
              {activePanel === "reports"  && <ReportsPanel     onBack={() => setActivePanel("write")} />}
            </div>

            {/* Tap NFC panel — only on write */}
            {activePanel === "write" && (
